...
function handler (req, res) { // handler za "response"; ta handler "handla" le datoteko index.html
    fs.readFile(__dirname + "/pwmbutton_12.html",
    function (err, data) {
        if (err) {
...
var board = new firmata.Board("/dev/ttyACM0",function(){
    ...
    board.pinMode(22, board.MODES.INPUT);
...
io.sockets.on("connection", function(socket) {  // od oklepaja ( dalje imamo argument funkcije on -> ob 'connection' se prenese argument t.j. funkcija(socket) 
    ...
    var timesArray = []; // ob povezavi klienta matriko brišemo
...
*/
    ...
    board.digitalRead(22, function(value) { // funkcija se aktivira le, kadar se spremeni stanje; sicer bi bilo 1M čitanj na sekundo
